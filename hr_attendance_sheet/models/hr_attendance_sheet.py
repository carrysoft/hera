    ...
    def get_attendances(self):
        for att_sheet in self:
            ...
            def get_work_intervals(calender, day_start, day_end):
                ...
                import datetime
                day_date1 = day_start.date()
                working_intervals = []
                for att in calender._get_day_attendances(day_date1,day_start.replace(hour=0, minute=0, second=0).time(),day_end.time()):
...
#                day_str = day.strftime('%A')
                day_str = dia_semana[datetime.weekday(day)]
